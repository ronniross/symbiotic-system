...
// Clean and setup active directory for the current session
...
// Scans checkpoints to figure out what sessions exist and loads stats
export function scanCheckpointStats() {
    ...
    const mainModelDir = path.join(CHECKPOINTS_DIR, MAIN_MODEL_NAME);
    if (fs.existsSync(mainModelDir)) {
        const sessionDirs = fs.readdirSync(mainModelDir).filter(d => d.startsWith('session_'));
        ...
        for (const sDir of sessionDirs) {
            ...
            const memPath = path.join(mainModelDir, sDir, 'memories');
            if (fs.existsSync(memPath)) {
                stats.inferences += fs.readdirSync(memPath).filter(f => f.endsWith('.json')).length;
            }
            if (fs.existsSync(path.join(mainModelDir, sDir, 'memory_graph.json'))) {
                stats.graphs++;
            }
        }
    }
...
export async function initMemory(startMode) {
    ...
    if (startMode === 'c' && checkpointStats.maxSession > 0) {
        currentSessionNum = checkpointStats.maxSession + 1;
        const modelDir = path.join(CHECKPOINTS_DIR, MAIN_MODEL_NAME);
        const sessionDirs = fs.readdirSync(modelDir).filter(d => d.startsWith('session_'));
        ...
        for (const sDir of sessionDirs) {
            const sNum = parseInt(sDir.split('_')[1]);
            const memPath = path.join(modelDir, sDir, 'memories');
            
            // Load integrated memories into RAM Pool
            if (fs.existsSync(memPath)) {
            ...
            // Keep track of latest graph
            const graphPath = path.join(modelDir, sDir, 'memory_graph.json');
        ...
        syncString = `[MEMORY] Synced ${loadedMemories} integrated engrams to Subconscious RAM from ${sessionDirs.length} past sessions.`;
    ...
    try {
        ...
        return { active: true, embedName: EMBED_MODEL_NAME, embedHash, sessionNum: currentSessionNum, stats: checkpointStats, syncString };
    } catch (e) {
        console.error(`[CRITICAL] Failed to load Embedding model:`, e);
        return { active: false, sessionNum: currentSessionNum, stats: checkpointStats, syncString: "[MEMORY] Failed to load embeddings." };
    }
}

// Archives the active session to the checkpoint folder upon Unload/Shutdown
export function archiveSessionToCheckpoint(sessionNum, startMode) {
    if (!fs.existsSync(ACTIVE_MEMORIES)) return null;
...
export function saveRawEvent(prompt, response, promptHash, responseHash, tick) {
    ...
    const eventData = { id, timestamp: new Date().toISOString(), tick, promptHash, responseHash, prompt, response, status: 'RAW' };
...
// Core retrieval & dreaming functions remain largely the same, just pointing to ACTIVE_MEMORIES
...
export async function processDreaming(onProgress) {
    ...
    for (let i = 0; i < memoryPool.length; i++) {
        ...
        if (evA.status === 'EMBEDDED') {
            ...
            // Immediately sync to RAM pool upon integration
