...
// FIXED: Removed generateFileHash from llm.js import
import { initModel, generateChat, clearHistory, contractState } from './src/llm.js';
import { timeline } from './src/timeline.js';
import { initMemory, saveRawEvent, getMemoryStats, processDreaming, retrieveSubconscious, archiveSessionToCheckpoint } from './src/memory.js';

// FIXED: Added generateFileHash to the utils.js import
...
// --- NEW API: Status Route for UI Header ---
...
app.post('/api/chat', async (req, res) => {
    ...
    try {
        ...
        if (contractState.path && fs.existsSync(contractState.path)) {
            // generateFileHash is now correctly called from utils.js
            currentContractHash = await generateFileHash(contractState.path);
            isValid = (currentContractHash === contractState.hash);
        }

        res.write(`data: ${JSON.stringify({ metadata: { promptHash, responseHash, contractHash: currentContractHash, isValid } })}\n\n`);
...
app.post('/api/shutdown', async (req, res) => {
    ...
    console.log('[SHUTDOWN] Archiving Session and Generating Closing Manifest...');
    let archiveDirStr = "None (Memory Disabled)";
    if (memoryConfig.active) {
        const archivedPath = archiveSessionToCheckpoint(memoryConfig.sessionNum, startMode);
        if (archivedPath) archiveDirStr = path.relative(__dirname, archivedPath);
    }

    const endManifest = await generateSystemManifest(path.join(__dirname, '..'));

    // Write massive Closing Log
    const closingLog = `\n==================================================
...
Memory Archive Location: ${archiveDirStr}
...
async function bootSystem() {
    try {
        ...
        bootManifest = await generateSystemManifest(path.join(__dirname, '..'));
