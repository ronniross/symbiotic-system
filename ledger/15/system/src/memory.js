            ...
            if (fs.existsSync(memPath)) {
                ...
                for (const f of files) {
                    ...
                    if (data.status === 'INTEGRATED') {
                        data.session = sNum; // TAG WITH SESSION FOR VISUALIZATION
                        ramPool.push(data);
                        loadedMemories++;
                    }
                }
...
export function saveRawEvent(prompt, response, promptHash, responseHash, tick) {
    ...
    const eventData = { id, session: activeSessionNumber, timestamp: new Date().toISOString(), tick, promptHash, responseHash, prompt, response, status: 'RAW' };
...
export function getMemoryMap() {
    const integrated = ramPool.map(m => ({ id: m.id, session: m.session || activeSessionNumber }));
    let pending = [];
    if (fs.existsSync(ACTIVE_MEMORIES)) {
        const files = fs.readdirSync(ACTIVE_MEMORIES).filter(f => f.endsWith('.json'));
        for (const f of files) {
            ...
            if (data.status === 'RAW' || data.status === 'EMBEDDED') {
                pending.push({ id: data.id, session: data.session || activeSessionNumber });
            }
        }
    }
