...
window.currentActiveModel = "Unknown";
...
async function fetchBootInfo() {
    try {
        ...
        if (data.modelName) {
            window.currentActiveModel = data.modelName;
...
async function sendMessage() {
    ...
    try {
                ...
                if (line.startsWith('data: ')) {
                    ...
                    try {
                        ...
                        if (parsed.recall) {
                            ...
                            // Turn ON the highlight continuously
                            if (parsed.recalledIds && parsed.recalledIds.length > 0) {
                                MemoryViz.startHighlight(parsed.recalledIds);
                            }
                        }
                        ...
                        if (parsed.metadata) {
                            ...
                            if (parsed.metadata.eventId) {
                                MemoryViz.addPending(parsed.metadata.eventId, window.currentActiveModel);
                            }
                        }
                    } catch (e) {}
                }
        ...
        // Turn OFF the highlight ONLY when the stream fully completes.
        MemoryViz.stopHighlight(); 
