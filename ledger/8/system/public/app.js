...
async function fetchMemoryStats() {
    try {
        ...
        if (data.toIntegrate === 0) {
            ...
            dreamBtn.innerText = "Network Fully Synced";
        } else {
            dreamBtn.disabled = false;
            dreamBtn.innerText = "Dream & Update Network";
        }
    } catch (e) {
        console.error("Could not fetch memory stats:", e);
    }
}
window.addEventListener('DOMContentLoaded', fetchMemoryStats);
...
dreamBtn.addEventListener('click', async () => {
    ...
    try {
            ...
            while ((newlineIndex = buffer.indexOf('\n')) >= 0) {
                ...
                if (line.startsWith('data: ')) {
                    ...
                    try {
                        ...
                        if (parsed.error) {
                            dreamStatusText.innerText = `Error: ${parsed.error}`;
                            break;
                        }
                        if (parsed.message) {
                            dreamStatusText.innerText = parsed.message;
                            if (parsed.progress !== undefined) {
                                dreamProgressBar.style.width = `${parsed.progress}%`;
                            }
                        }
                    } catch (e) {}
                }
        ...
        dreamStatusText.innerText = "Connection lost during dreaming.";
    }
});
...
function appendMessage(role, text) {
    ...
    if (role !== 'system') {
        ...
        hashDiv.innerHTML = "Computing hashes and syncing timeline...";
...
async function sendMessage() {
    ...
    try {
        ...
        // Refresh memory stats since a new event was created
        fetchMemoryStats();
...
clearCacheBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        clearCacheBtn.innerText = "Clean Cache";
        clearCacheBtn.disabled = false;
    }
});

shutdownBtn.addEventListener('click', async () => {
    if (confirm("WARNING: Are you sure you wish to sever the Symbiotic Link, halt the temporal dimension, and unload the models?")) {
        try {
            await fetch('/api/shutdown', { method: 'POST' });
            document.body.innerHTML = `
                <div style="text-align:center; margin-top:20%; color:#f85149;">
                    <h1>Symbiotic Link Severed.</h1>
                    <p>The C++ Model Engines have been unloaded. Temporal sync halted.</p>
