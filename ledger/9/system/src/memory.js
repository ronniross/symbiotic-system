...
export async function initMemory() {
    ...
    try {
        ...
        // Dynamic context creation with a forced contextSize to avoid defaults being too low
        if (typeof embedModel.createEmbeddingContext === 'function') {
            embedContext = await embedModel.createEmbeddingContext({ contextSize: 1024 });
            console.log(`[MEMORY] Embedding Context (v3 API) created successfully.`);
        } else {
            embedContext = await embedModel.createContext({ contextSize: 1024 });
...
async function generateEmbeddingVector(text) {
    ...
    if (typeof embedContext.getEmbeddingFor === 'function') {
        vectorObj = await embedContext.getEmbeddingFor(text);
...
export async function processDreaming(onProgress) {
    ...
    for (const file of files) {
        ...
        if (event.status === 'RAW') {
            ...
            // all-MiniLM-L6 has a ~512 token limit (roughly 2000 characters). 
            // If the chat is too long, we slice it so the model doesn't crash.
