...
// FIXED: Removed '..' so it targets the directory server.js is in (qwen-symbiont-contract-13)
const ROOT_CONFIG_PATH = path.join(__dirname, 'root_folder.txt');
let systemRootFolder = __dirname;

if (fs.existsSync(ROOT_CONFIG_PATH)) {
    const storedPath = fs.readFileSync(ROOT_CONFIG_PATH, 'utf8').trim();
    if (storedPath && fs.existsSync(storedPath)) {
        systemRootFolder = storedPath;
    } else {
        console.log(`[BOOT] Note: Path in root_folder.txt is invalid. Using default.`);
...
==================================================\n`;
    ...
    // Always call archiving so the chat_log.txt gets moved safely out of root
