...
export async function initMemory() {
    ...
    try {
        ...
        // We REMOVED the forced context size. 
        // node-llama-cpp will now correctly read the native 512 limit from the model's GGUF metadata.
...
export async function processDreaming(onProgress) {
    ...
    for (const file of files) {
        ...
        if (event.status === 'RAW') {
            ...
            // --- UPDATED SAFETY TRUNCATION ---
            // 1500 characters safely guarantees we stay under the physical 512 token limit
            if (textToEmbed.length > 1500) {
                textToEmbed = textToEmbed.substring(0, 1500);
            }
